...
<html lang="en">
  <!--
   ! WiP.
   ! A Form to query the PROJECTS table, leads to an update form.
   +-->
  <head>
    <!--meta charset="UTF-8">
  ...
  if ($operation == 'query') { // Then do the query
    try {
      ...
      FROM
          PROJECTS AS PRJ
      WHERE
           UPPER(PRJ.PROJECT_ID) LIKE UPPER(\'%' . $name . '%\') OR
           UPPER(PRJ.PROJECT_NAME) LIKE UPPER(\'%' . $name . '%\')
      ORDER BY 1;'; // Order by PROJECT_ID
      ...
      echo("<h2>Passe-Coque Projects</h2>");
      ...
      <table cellspacing="20">
        <tr>
          <td style="border: none;">
            <form action="./_projects.02.php" method="get">
      ...
      echo "<tr><th class='no-rotate'><div><span>ID</span></div></th>" .
               "<th class='no-rotate'><div><span>Name</span></div></th>" .
               "<th class='no-rotate'><div><span>Description</span></div></th></tr>";
      while ($table = mysqli_fetch_array($result)) { // go through each row that was returned in $result
        echo(
          "<tr><td>" .
            urldecode($table[0]) . // Project ID
          "</td><td class='no-wrap'>" .
            /*mb_convert_encoding($table[1], 'UTF-8', 'ISO-8859-1') . //  utf8_encode*/ ($table[1]) . // Name (full) //
          "</td><td class='no-wrap-2'>" .
           $table[2] . // Description
          "</td><td>" .
            "<a href='./_projects.02.php?tx-id=" . $table[0] . "'>Edit</a>" .  // Edit Member //  target='PCUpdate'
          "</td></tr>\n"
        );
      }
      echo "</table>";
    ...
    <form action="" method="post">
      ...
      <table>
        <tr>
          <td valign="top">Name (part of project id or project name):</td><td><input type="text" name="full-name" size="40" placeholder="Your filter"></td>
        </tr>
        <tr>
