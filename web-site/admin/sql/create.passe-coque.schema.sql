...
ALTER TABLE THE_FLEET ADD CONSTRAINT CHK_CAT CHECK (CATEGORY IN ('CLUB', 'TO_GRAB', 'EX_BOAT', 'NONE', 'PARTNERS', 'FOR_SALE'));
...
CREATE TABLE IF NOT EXISTS PROJECT_CONTRIBUTORS (
    ...
    CONSTRAINT MEMBER_FK_DONOR FOREIGN KEY (DONOR_EMAIL) REFERENCES PASSE_COQUE_MEMBERS(EMAIL) ON DELETE CASCADE,
    CONSTRAINT DONOR_FK_PROJECT FOREIGN KEY (PROJECT_ID) REFERENCES PROJECTS(PROJECT_ID) ON DELETE CASCADE,
    CONSTRAINT PK_PRJ_DONOR PRIMARY KEY (PROJECT_ID, DONOR_EMAIL)
);

CREATE VIEW PROJECTS_AND_OWNERS AS
SELECT CONCAT(PCM.FIRST_NAME, ' ', PCM.LAST_NAME) AS OWNER,
       PRJ.PROJECT_ID,
       P.PROJECT_NAME
FROM PASSE_COQUE_MEMBERS PCM,
     PROJECTS P,
     PROJECT_OWNERS PRJ
WHERE PCM.EMAIL = PRJ.OWNER_EMAIL AND
      PRJ.PROJECT_ID = P.PROJECT_ID
ORDER BY PRJ.PROJECT_ID;

CREATE VIEW PROJECTS_DONORS AS
SELECT M.EMAIL,
...
FROM PROJECT_CONTRIBUTORS PC,
     PASSE_COQUE_MEMBERS M,
     PROJECTS P
WHERE M.EMAIL = PC.DONOR_EMAIL AND
...
INSERT INTO PROJECTS (PROJECT_ID, PROJECT_NAME) VALUES ('tx-02', 'Voyage en Patrimoine');
...
-- Dec-18, 2024
INSERT INTO PROJECT_CONTRIBUTORS (PROJECT_ID, DONOR_EMAIL) VALUES ('tx-05', '421jimmypahun@gmail.com');
...
CREATE TABLE IF NOT EXISTS TODO_LISTS (
    ...
    LINE_DESC VARCHAR(512) NOT NULL COMMENT 'Decription',
    ...
    ASSIGNED_TO VARCHAR(64) DEFAULT NULL COMMENT 'Assigned to (email)',
    ...
    CONSTRAINT TODO_FK_MEMBER FOREIGN KEY (ASSIGNED_TO) REFERENCES PASSE_COQUE_MEMBERS(EMAIL) ON DELETE SET NULL
